...
class SampleApp(tk.Tk):
    def __init__(self):
        ...
        self.label = tk.Label(self, text="MODIFY MY ACCOUNT", bg="#0E4F77", fg="white", font = ("Helvetica", 18), pady=20, padx=30)
    ...
    def submit(self):
        ...
        mycursor.execute("UPDATE USERS(nom, prenom, age, taille, poids, username, passwords, sexe) VALUES ( %s, %s, %s, %s, %s, %s, %s, %s) WHERE username = %s",(self.name.get(), self.surname.get(), self.age.get(), self.height.get(), self.weight.get(), self.username.get(), self.password.get(), self.sexe.get(), self.username.get()))
        mydb.commit()
